...
function pkuxkx.gps.getLocation()
    ...
    -- 特殊房间定位处理
    if pkuxkx.gps.current.RoomName == "黑沼" then
        exe("s;e;n;w;s;s;w;")
        return tempTimer(0.8, [[pkuxkx.gps.getLocation()]])
    end
    if pkuxkx.gps.current.RoomName == "茶花林" then
        exe("w;n;w;s;s;e;e;w;w;")
        return tempTimer(0.8, [[pkuxkx.gps.getLocation()]])
    end
    if pkuxkx.gps.current.RoomName == "长江渡船" or pkuxkx.gps.current.RoomName == "黄河渡船" then
        return tempTimer(0.8, [[pkuxkx.gps.getLocation()]])
    end
    -- 特殊 end
...
function pkuxkx.gps.goRoom(roomid)
    ...
    pkuxkx.gps.walk.failEvent = function()
        if pkuxkx.gps.walk.failtimes and pkuxkx.gps.walk.failtimes > 10 then
            return common.error("行走出现异常, 强制停止! 目标Room为: " .. pkuxkx.path.targetroom)
        else
            -- 中止行走
            pkuxkx.gps.stop()
            -- 强制重新定位,
...
-- ---------------------------------------------------------------
-- 添加已经经过的房间至 pkuxkx.gps.walk.passedrooms
-- ---------------------------------------------------------------
...
function _Direction_IsAvailable(thisdirection)
    ...
    if gps_mode == 0 and thisdirection.money ~= nil and tonumber(thisdirection.money) > 5000 then
        isAvailable = false
    end
    -- 短距离不坐车
    if gps_mode == 0 and string.contain(thisdirection.special, "!zuoche") and tonumber(thisdirection.money) < 450 then
        isAvailable = false
    end
    ----------------
    -- 判断 武功wugong
...
-- 到达提醒
-- ---------------------------------------------------------------
function pkuxkx.gps.arrivereminder()
    common.log("已到达目的地。")
end
-- ---------------------------------------------------------------
-- 包装 go 方法
...
function pkuxkx.gps.go(arg1, arg2)
    ...
    if pkuxkx.gps.arriveEvent == nil then
        pkuxkx.gps.arriveEvent = pkuxkx.gps.arrivereminder
    end
...
function pkuxkx.gps.gotodoExtend(area, room, script)
    ...
    if pkuxkx.gps.goArgs_aftergoEvent ~= nil then
        pkuxkx.gps.arriveEvent = pkuxkx.gps.goArgs_aftergoEvent
    end
