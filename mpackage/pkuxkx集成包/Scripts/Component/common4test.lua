...
function string.contain(str, character)
    -- print("\n 当前命令为: "..str)
    if not str or not character or string.match(str, character) == nil then
        return false
    else
        return true
    end
...
-- ----------------------------------------------------------
-- 自定义实现 lua string.trim  (已弃用, 在mudlet中有自定义实现的string.trim, 重名,故弃用, 可用mudlet自带的string.trim)
-- ----------------------------------------------------------
